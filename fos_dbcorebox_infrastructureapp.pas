unit FOS_DBCOREBOX_INFRASTRUCTUREAPP;
...
type
  ...
  { TFRE_DBCOREBOX_INFRASTRUCTURE_APP }

  TFRE_DBCOREBOX_INFRASTRUCTURE_APP=class(TFRE_DB_APPLICATION)
  private
...
procedure Register_DB_Extensions;
begin
  GFRE_DBI.RegisterObjectClassEx(TFRE_DBCOREBOX_INFRASTRUCTURE_APP);
  GFRE_DBI.Initialize_Extension_Objects;
end;

{ TFRE_DBCOREBOX_INFRASTRUCTURE_APP }

procedure TFRE_DBCOREBOX_INFRASTRUCTURE_APP.SetupApplicationStructure;
begin
...
function TFRE_DBCOREBOX_INFRASTRUCTURE_APP.InstallAppDefaults(const conn: IFRE_DB_SYS_CONNECTION): TFRE_DB_Errortype;
...
procedure TFRE_DBCOREBOX_INFRASTRUCTURE_APP._UpdateSitemap( const session: TFRE_DB_UserSession);
var
...
procedure TFRE_DBCOREBOX_INFRASTRUCTURE_APP.MySessionInitialize(  const session: TFRE_DB_UserSession);
begin
...
procedure TFRE_DBCOREBOX_INFRASTRUCTURE_APP.MySessionPromotion(  const session: TFRE_DB_UserSession);
begin
  inherited MySessionPromotion(session);
  _UpdateSitemap(session);
end;

function TFRE_DBCOREBOX_INFRASTRUCTURE_APP.CFG_ApplicationUsesRights: boolean;
begin
  result := true;
end;

function TFRE_DBCOREBOX_INFRASTRUCTURE_APP._ActualVersion: TFRE_DB_String;
begin
  Result := '1.0';
end;

class procedure TFRE_DBCOREBOX_INFRASTRUCTURE_APP.RegisterSystemScheme( const scheme: IFRE_DB_SCHEMEOBJECT);
begin
