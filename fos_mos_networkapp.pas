...
type
  ...
  { TFOS_CITYCOM_DNS_MOD }

  TFOS_CITYCOM_DNS_MOD = class (TFRE_DB_APPLICATION_MODULE)
  protected
    class procedure RegisterSystemScheme                (const scheme: IFRE_DB_SCHEMEOBJECT); override;
...
begin
  ...
  GFRE_DBI.RegisterObjectClassEx(TFOS_CITYCOM_DNS_MOD);
...
class procedure TFOS_CITYCOM_DNS_MOD.RegisterSystemScheme(const scheme: IFRE_DB_SCHEMEOBJECT);
begin
  inherited RegisterSystemScheme(scheme);
  scheme.SetParentSchemeByName('TFRE_DB_APPLICATION_MODULE');
end;

procedure TFOS_CITYCOM_DNS_MOD.SetupAppModuleStructure;
begin
  inherited SetupAppModuleStructure;
  InitModuleDesc('dns_description')
end;

class procedure TFOS_CITYCOM_DNS_MOD.InstallDBObjects(const conn: IFRE_DB_SYS_CONNECTION; var currentVersionId: TFRE_DB_NameType; var newVersionId: TFRE_DB_NameType);
begin
  newVersionId:='1.0';
  if currentVersionId='' then begin
    currentVersionId := '1.0';
...
function TFOS_CITYCOM_DNS_MOD._AddModifyResourceRecord(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION; const isModify:Boolean): IFRE_DB_Object;
var
...
begin
  ...
  if isModify then begin
    ...
    diagCap:=app.FetchAppTextShort(ses,'resource_record_modify_diag_cap');
  end else begin
    sf.AddParam.Describe('networkDomainId',input.FieldPath('dependency.uid_ref.filtervalues').AsStringItem[0]);
    diagCap:=app.FetchAppTextShort(ses,'resource_record_create_diag_cap');
  end;
...
procedure TFOS_CITYCOM_DNS_MOD.MySessionInitializeModule(const session: TFRE_DB_UserSession);
var
  ...
  if session.IsInteractiveSession then begin
    ...
    if session.IsInteractiveSession then begin
      ...
      with transform do begin
        AddOneToOnescheme('name','name',app.FetchAppTextShort(session,'grid_network_domains_name'),dt_string,true,true);
        AddMatchingReferencedField('default','host','default',app.FetchAppTextShort(session,'grid_network_domains_default'));
        AddMatchingReferencedField('default','type','default_type',app.FetchAppTextShort(session,'grid_network_domains_default_type'));
      end;
    ...
    GFRE_DBI.NewObjectIntf(IFRE_DB_SIMPLE_TRANSFORM,transform);
    with transform do begin
      AddOneToOnescheme('host','host',app.FetchAppTextShort(session,'grid_records_host'),dt_string,true,true);
      AddOneToOnescheme('type','type',app.FetchAppTextShort(session,'grid_records_type'),dt_string);
      AddOneToOnescheme('value','value',app.FetchAppTextShort(session,'grid_records_value'),dt_string);
      AddOneToOnescheme('ttl','ttl',app.FetchAppTextShort(session,'grid_records_ttl'),dt_number);
    end;
...
function TFOS_CITYCOM_DNS_MOD.WEB_Content(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION): IFRE_DB_Object;
var
...
begin
  ...
  if conn.sys.CheckClassRight4MyDomain(sr_STORE,TFOS_DB_NETWORK_DOMAIN) and conn.sys.CheckClassRight4MyDomain(sr_STORE,TFOS_DB_DNS_RESOURCE_RECORD) then begin
    domains_grid.AddButton.Describe(CWSF(@WEB_AddNetworkDomain),'',app.FetchAppTextShort(ses,'tb_create_network_domain'),app.FetchAppTextHint(ses,'tb_create_network_domain'));
  end;
  if conn.sys.CheckClassRight4MyDomain(sr_DELETE,TFOS_DB_NETWORK_DOMAIN) and conn.sys.CheckClassRight4MyDomain(sr_DELETE,TFOS_DB_DNS_RESOURCE_RECORD) then begin
    domains_grid.AddButton.Describe(CWSF(@WEB_NetworkDomainDelete),'',app.FetchAppTextShort(ses,'tb_delete_network_domain'),app.FetchAppTextHint(ses,'tb_delete_network_domain'),fdgbd_single);
  end;
  ...
  domain_content.AddSection.Describe(CWSF(@WEB_DomainDetailsContent),app.FetchAppTextShort(ses,'network_domain_content_section_details'),1);
  domain_content.AddSection.Describe(CWSF(@WEB_DomainRecordsContent),app.FetchAppTextShort(ses,'network_domain_content_section_records'),2);

  Result:=TFRE_DB_LAYOUT_DESC.create.Describe().SetLayout(domains_grid,domain_content,nil,nil,nil,true,1,1);
end;

function TFOS_CITYCOM_DNS_MOD.WEB_DomainDetailsContent(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION): IFRE_DB_Object;
var
...
begin
  ...
  if ses.GetSessionModuleData(ClassName).FieldExists('selectedDomain') and (ses.GetSessionModuleData(ClassName).Field('selectedDomain').ValueCount=1) then begin
    ...
    form.AddSchemeFormGroup(rr_scheme.GetInputGroup('main'),ses,false,false,'dns1',true).SetCaption(app.FetchAppTextShort(ses,'network_domain_edit_dns1'));
    form.AddSchemeFormGroup(rr_scheme.GetInputGroup('main'),ses,false,false,'dns2',false).SetCaption(app.FetchAppTextShort(ses,'network_domain_edit_dns2'));
    ...
    for i := 0 to nw_domain.Field('records').ValueCount - 1 do begin
      ...
      if ((rrecord.Field('type').AsString='A') or (rrecord.Field('type').AsString='AAAA')) then begin
        defaultStore.AddEntry.Describe(rrecord.Field('host').AsString + ' ' + app.FetchAppTextShort(ses,'network_domain_edit_default_postfix_val_'+rrecord.Field('type').AsString),rrecord.UID_String);
      end;
    end;

    defaultG:=form.AddGroup.Describe(app.FetchAppTextShort(ses,'network_domain_edit_default'));
    defaultG.AddChooser.Describe(app.FetchAppTextShort(ses,'network_domain_edit_default_chooser'),'default_host',defaultStore);
    ...
    res:=TFRE_DB_HTML_DESC.create.Describe(app.FetchAppTextShort(ses,'info_domain_details_select_one'));
  end;
  res.contentId:='NW_DOMAIN_DETAILS';
  Result:=res;
end;

function TFOS_CITYCOM_DNS_MOD.WEB_DomainRecordsContent(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION): IFRE_DB_Object;
var
...
begin
  ...
  if conn.sys.CheckClassRight4MyDomain(sr_UPDATE,TFOS_DB_NETWORK_DOMAIN) and  conn.sys.CheckClassRight4MyDomain(sr_STORE,TFOS_DB_DNS_RESOURCE_RECORD) then begin
    records_grid.AddButton.DescribeManualType('add_record',CWSF(@WEB_AddResourceRecord),'',app.FetchAppTextShort(ses,'tb_create_resource_record'),app.FetchAppTextHint(ses,'tb_create_resource_record'),not (ses.GetSessionModuleData(ClassName).FieldExists('selectedDomain') and (ses.GetSessionModuleData(ClassName).Field('selectedDomain').ValueCount=1)));
  end;
  if conn.sys.CheckClassRight4MyDomain(sr_UPDATE,TFOS_DB_NETWORK_DOMAIN) and conn.sys.CheckClassRight4MyDomain(sr_DELETE,TFOS_DB_DNS_RESOURCE_RECORD) then begin
    records_grid.AddButton.Describe(CWSF(@WEB_ResourceRecordDelete),'',app.FetchAppTextShort(ses,'tb_delete_resource_record'),app.FetchAppTextHint(ses,'tb_delete_resource_record'),fdgbd_single);
  end;
  if conn.sys.CheckClassRight4MyDomain(sr_UPDATE,TFOS_DB_NETWORK_DOMAIN) and conn.sys.CheckClassRight4MyDomain(sr_UPDATE,TFOS_DB_DNS_RESOURCE_RECORD) then begin
    records_grid.AddButton.Describe(CWSF(@WEB_ModifyResourceRecord),'',app.FetchAppTextShort(ses,'tb_modify_resource_record'),app.FetchAppTextHint(ses,'tb_modify_resource_record'),fdgbd_single);
  end;

  Result:=records_grid;
end;

function TFOS_CITYCOM_DNS_MOD.WEB_NetworkDomainsMenu(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION): IFRE_DB_Object;
var
...
begin
  if conn.sys.CheckClassRight4MyDomain(sr_DELETE,TFOS_DB_NETWORK_DOMAIN) and conn.sys.CheckClassRight4MyDomain(sr_DELETE,TFOS_DB_DNS_RESOURCE_RECORD) then begin
    ...
    res.AddEntry.Describe(app.FetchAppTextShort(ses,'cm_delete_network_domain'),'',func);
    Result:=res;
  end else begin
...
function TFOS_CITYCOM_DNS_MOD.WEB_NetworkDomainsSC(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION): IFRE_DB_Object;
var
...
function TFOS_CITYCOM_DNS_MOD.WEB_AddNetworkDomain(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION): IFRE_DB_Object;
var
  scheme,rr_scheme: IFRE_DB_SchemeObject;
  res             : TFRE_DB_FORM_DIALOG_DESC;
begin
  ...
  GetSystemScheme(TFOS_DB_NETWORK_DOMAIN,scheme);
  GetSystemScheme(TFOS_DB_DNS_RESOURCE_RECORD,rr_scheme);
  res:=TFRE_DB_FORM_DIALOG_DESC.create.Describe(app.FetchAppTextShort(ses,'network_domain_create_diag_cap'),600);
  res.AddSchemeFormGroup(scheme.GetInputGroup('main'),ses,false,false,'domain');

  res.AddSchemeFormGroup(rr_scheme.GetInputGroup('main'),ses,false,false,'dns1',true).SetCaption(app.FetchAppTextShort(ses,'network_domain_create_dns1'));
  res.AddSchemeFormGroup(rr_scheme.GetInputGroup('main'),ses,false,false,'dns2',false).SetCaption(app.FetchAppTextShort(ses,'network_domain_create_dns2'));
  res.AddSchemeFormGroup(rr_scheme.GetInputGroup('main'),ses,false,false,'default',false).SetCaption(app.FetchAppTextShort(ses,'network_domain_create_default'));
...
function TFOS_CITYCOM_DNS_MOD.WEB_CreateNetworkDomain(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION): IFRE_DB_Object;
var
...
begin
  ...
  if domainColl.ExistsIndexed(input.FieldPath('data.domain.name').AsString) then begin
    Result:=TFRE_DB_MESSAGE_DESC.create.Describe(app.FetchAppTextShort(ses,'network_domain_create_error_exists_cap'),app.FetchAppTextShort(ses,'network_domain_create_error_exists_msg'),fdbmt_error);
    exit;
  end;
...
function TFOS_CITYCOM_DNS_MOD.WEB_UpdateNetworkDomain(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION): IFRE_DB_Object;
var
  resource      : TFOS_DB_DNS_RESOURCE_RECORD;
  schemeObject  : IFRE_DB_SchemeObject;
  schemeObjectRR: IFRE_DB_SchemeObject;
...
function TFOS_CITYCOM_DNS_MOD.WEB_NetworkDomainDelete(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION): IFRE_DB_Object;
var
  sf     : TFRE_DB_SERVER_FUNC_DESC;
  cap,msg: String;
...
begin
  ...
  cap:=app.FetchAppTextShort(ses,'network_domain_delete_diag_cap');

  CheckDbResult(conn.Fetch(FREDB_String2Guid(input.Field('selected').AsStringArr[0]),domain));
  msg:=StringReplace(app.FetchAppTextShort(ses,'network_domain_delete_diag_msg'),'%nw_domain_str%',domain.Field('name').AsString,[rfReplaceAll]);
  Result:=TFRE_DB_MESSAGE_DESC.create.Describe(cap,msg,fdbmt_confirm,sf);
end;

function TFOS_CITYCOM_DNS_MOD.WEB_NetworkDomainDeleteConfirmed(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION): IFRE_DB_Object;
var
  i,j          : NativeInt;
  nwDomainId   : TGuid;
  domain       : IFRE_DB_Object;
...
function TFOS_CITYCOM_DNS_MOD.WEB_AddResourceRecord(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION): IFRE_DB_Object;
begin
  ...
  if not (input.FieldPathExists('dependency.uid_ref.filtervalues') and (input.FieldPath('dependency.uid_ref.filtervalues').ValueCount=1)) then
    raise EFRE_DB_Exception.Create(app.FetchAppTextShort(ses,'Error: Network Domain Id is missing.'));

  Result:=_AddModifyResourceRecord(input,ses,app,conn,false);
end;

function TFOS_CITYCOM_DNS_MOD.WEB_ModifyResourceRecord(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION): IFRE_DB_Object;
begin
...
function TFOS_CITYCOM_DNS_MOD.WEB_StoreResourceRecord(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION): IFRE_DB_Object;
var
  domain        : IFRE_DB_Object;
  resource      : TFOS_DB_DNS_RESOURCE_RECORD;
  schemeObject  : IFRE_DB_SchemeObject;
...
begin
  ...
  if not (input.FieldExists('networkDomainId') or input.FieldExists('rrecordId')) then
    raise EFRE_DB_Exception.Create(app.FetchAppTextShort(ses,'Error: Network Domain Id or Resource Record Id has to be passed to WEB_StoreResourceRecord.'));
...
function TFOS_CITYCOM_DNS_MOD.WEB_ResourceRecordDelete(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION): IFRE_DB_Object;
var
  sf         : TFRE_DB_SERVER_FUNC_DESC;
  cap,msg    : String;
  rrecord    : IFRE_DB_Object;
...
begin
  ...
  cap:=app.FetchAppTextShort(ses,'resource_record_delete_diag_cap');

  CheckDbResult(conn.Fetch(FREDB_String2Guid(input.Field('selected').AsStringArr[0]),rrecord));
  msg:=StringReplace(app.FetchAppTextShort(ses,'resource_record_delete_diag_msg'),'%rr_str%',rrecord.Field('host').AsString,[rfReplaceAll]);
  Result:=TFRE_DB_MESSAGE_DESC.create.Describe(cap,msg,fdbmt_confirm,sf);
end;

function TFOS_CITYCOM_DNS_MOD.WEB_ResourceRecordDeleteConfirmed(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION): IFRE_DB_Object;
var
  i       : NativeInt;
  recordId: TGuid;
  domain  : IFRE_DB_Object;
...
function TFOS_CITYCOM_DNS_MOD.WEB_ResourceRecordsMenu(const input: IFRE_DB_Object; const ses: IFRE_DB_Usersession; const app: IFRE_DB_APPLICATION; const conn: IFRE_DB_CONNECTION): IFRE_DB_Object;
var
  res : TFRE_DB_MENU_DESC;
  func: TFRE_DB_SERVER_FUNC_DESC;
begin
  if input.FieldPathExists('dependency.uid_ref.filtervalues') and
    ...
    if conn.sys.CheckClassRight4MyDomain(sr_UPDATE,TFOS_DB_NETWORK_DOMAIN) and conn.sys.CheckClassRight4MyDomain(sr_DELETE,TFOS_DB_DNS_RESOURCE_RECORD) then begin
      ...
      res.AddEntry.Describe(app.FetchAppTextShort(ses,'cm_delete_resource_record'),'',func);
    end;
    if conn.sys.CheckClassRight4MyDomain(sr_UPDATE,TFOS_DB_NETWORK_DOMAIN) and conn.sys.CheckClassRight4MyDomain(sr_UPDATE,TFOS_DB_DNS_RESOURCE_RECORD) then begin
      func:=CWSF(@WEB_ModifyResourceRecord);
      func.AddParam.Describe('selected',input.Field('selected').AsStringArr);
      res.AddEntry.Describe(app.FetchAppTextShort(ses,'cm_modify_resource_record'),'',func);
    end;
    Result:=res;
  end else begin
    Result:=GFRE_DB_NIL_DESC;
...
begin
  ...
  AddApplicationModule(TFOS_CITYCOM_DNS_MOD.create);
end;
...
begin
  ...
  FREDB_SiteMap_AddRadialEntry(SiteMapData,'NETWORK/DNS',FetchAppTextShort(session,'sitemap_dns'),'images_apps/citycom_network/dns_white.svg',TFOS_CITYCOM_DNS_MOD.Classname,0,conn.sys.CheckClassRight4MyDomain(sr_FETCH,TFOS_CITYCOM_DNS_MOD));
...
begin
  ...
  if (currentVersionId='') then begin
    ...
    //TFOS_CITYCOM_DNS_MOD;
    CreateAppText(conn,'dns_description','DNS','DNS','DNS');

    CreateAppText(conn,'grid_network_domains_name','Name');
    CreateAppText(conn,'grid_network_domains_default','Default');
    CreateAppText(conn,'grid_network_domains_default_type','Type');
    CreateAppText(conn,'grid_records_host','Host');
    CreateAppText(conn,'grid_records_type','Type');
    CreateAppText(conn,'grid_records_value','Value');
    CreateAppText(conn,'grid_records_ttl','TTL');

    CreateAppText(conn,'tb_create_network_domain','Add');
    CreateAppText(conn,'tb_delete_network_domain','Delete');
    CreateAppText(conn,'cm_delete_network_domain','Delete');

    CreateAppText(conn,'tb_create_resource_record','Add');
    CreateAppText(conn,'tb_delete_resource_record','Delete');
    CreateAppText(conn,'tb_modify_resource_record','Modify');
    CreateAppText(conn,'cm_delete_resource_record','Delete');
    CreateAppText(conn,'cm_modify_resource_record','Modify');

    CreateAppText(conn,'network_domain_create_diag_cap','New Domain');
    CreateAppText(conn,'network_domain_create_dns1','DNS 1');
    CreateAppText(conn,'network_domain_create_dns2','DNS 2');
    CreateAppText(conn,'network_domain_create_default','Default');

    CreateAppText(conn,'network_domain_edit_dns1','DNS 1');
    CreateAppText(conn,'network_domain_edit_dns2','DNS 2');
    CreateAppText(conn,'network_domain_edit_default','Default');
    CreateAppText(conn,'network_domain_edit_default_chooser','Host');
    CreateAppText(conn,'network_domain_edit_default_postfix_val_A','(A)');
    CreateAppText(conn,'network_domain_edit_default_postfix_val_AAAA','(AAAA)');

    CreateAppText(conn,'network_domain_create_error_exists_cap','Create Network Domain');
    CreateAppText(conn,'network_domain_create_error_exists_msg','Network Domain already exists. Please choose a different network domain name.');
    CreateAppText(conn,'network_domain_delete_diag_cap','Delete Network Domain');
    CreateAppText(conn,'network_domain_delete_diag_msg','Delete Network Domain %nw_domain_str%?');

    CreateAppText(conn,'network_domain_content_section_details','Details');
    CreateAppText(conn,'network_domain_content_section_records','Records');

    CreateAppText(conn,'resource_record_create_diag_cap','Create Resource Record');
    CreateAppText(conn,'resource_record_modify_diag_cap','Modify Resource Record');
    CreateAppText(conn,'resource_record_delete_diag_cap','Delete Resource Record');
    CreateAppText(conn,'resource_record_delete_diag_msg','Delete Resource Record %rr_str%?');

    CreateAppText(conn,'info_domain_details_select_one','Please select a network domain to get detailed information about it.');
...
class procedure TFOS_CITYCOM_NETWORK_APP.InstallDBObjects4Domain(const conn: IFRE_DB_SYS_CONNECTION; currentVersionId: TFRE_DB_NameType; domainUID: TGUID);
begin
  ...
  if currentVersionId='' then begin
    ...
    CheckDbResult(conn.AddRolesToGroup('NETWORKADMINS',domainUID,TFRE_DB_StringArray.Create(
      ...
      TFOS_CITYCOM_DNS_MOD.GetClassRoleNameFetch
...
class procedure TFOS_CITYCOM_NETWORK_APP.InstallUserDBObjects4Domain(const conn: IFRE_DB_CONNECTION; currentVersionId: TFRE_DB_NameType; domainUID: TGUID);
var
...
class procedure TFOS_CITYCOM_NETWORK_APP.InstallUserDBObjects4SysDomain(const conn: IFRE_DB_CONNECTION; currentVersionId: TFRE_DB_NameType; domainUID: TGUID);
begin
